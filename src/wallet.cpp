...
void CWallet::ExportPeercoinKeys(int &nExportedCount, int &nErrorCount)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, mapAddressBook)
    {
        ...
        if (address.IsScript())
        {
            CKeyID hash;
            CScript script;
            if (!address.GetKeyID(hash) || !GetCScript(hash, script))
            {
            ...
            txnouttype type;
            std::vector<CTxDestination> vAddresses;
            int nRequired;
            if (!ExtractDestinations(script, type, vAddresses, nRequired))
            {
        ...
        {
            CKeyID keyID;
            if (!address.GetKeyID(keyID) || !GetSecret(keyID, vchSecret, fCompressed))
            {
